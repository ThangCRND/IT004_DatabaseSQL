...
FROM KHOA
WHERE NGTHANHLAP = ( SELECT MIN(NGTHANHLAP) 
                     FROM KHOA )
...
--Mở rộng : có bao nhiêu giảng viên chưa là GS, PGS

SELECT COUNT (MAGV) AS 'Chưa là GS,PGS'
FROM GIAOVIEN
WHERE HOCHAM NOT IN ( 'GS','PGS')


--THÊM:Có bao nhiêu giảng viên chưa có học hàm

SELECT COUNT (MAGV) AS 'Chưa có học hàm'
FROM GIAOVIEN
WHERE HOCHAM = 'Null' 
...
--TH1: Chỉ có bảng GIAOVIEN:
SELECT MAKHOA, HOCVI, COUNT(MAGV) AS 'Số Giáo viên'
FROM GIAOVIEN
...
--CÂU HOÀN CHỈNH:
--Kết 2 bảng để lấy TENKHOA :

SELECT GIAOVIEN.MAKHOA, TENKHOA, HOCVI, COUNT(MAGV) AS 'Số Giáo viên'
FROM GIAOVIEN
...
--THÊM:21.1 Cập nhật PTS thành TS , TS thành TSKH
...
SELECT HOCVI FROM GIAOVIEN



--22. Mỗi môn học thống kê số lượng học viên theo kết quả (đạt và không đạt).

SELECT MONHOC.MAMH, TENMH, KQUA, COUNT(MAHV) AS 'Số học viên'
FROM KETQUATHI
INNER JOIN MONHOC ON MONHOC.MAMH = KETQUATHI.MAMH
GROUP BY MONHOC.MAMH,TENMH, KQUA
ORDER BY MAMH

--THÊM:22.1 Thống kê có bao nhiêu học viên đạt, ko đạt theo từng lớp

SELECT HOCVIEN.MALOP, MONHOC.MAMH, TENMH, KQUA, COUNT(HOCVIEN.MAHV) AS 'Số học viên'
FROM KETQUATHI
INNER JOIN MONHOC ON MONHOC.MAMH = KETQUATHI.MAMH
INNER JOIN HOCVIEN ON HOCVIEN.MAHV = KETQUATHI.MAHV
GROUP BY MALOP, MONHOC.MAMH,TENMH, KQUA
ORDER BY MALOP
...
WHERE MAGV IN ( SELECT MAGVCN
                FROM LOP AS L
				WHERE MAGVCN = GV.MAGV
				AND EXISTS ( 
				             SELECT MAGV 
				             FROM GIANGDAY
							 WHERE MAGV = GV.MAGV AND MALOP = L.MALOP 
						   )
			  )
...
SELECT MAHV
FROM ( 
      SELECT MAHV, COUNT (MAHV) AS SOLUONG
      FROM ( 
	         SELECT MAHV, MAMH
			 FROM KETQUATHI
			 ...
			 SELECT MAHV, MAMH
			 FROM KETQUATHI
			 WHERE KQUA = 'Dat' AND LANTHI >= 2 ) AS KQ
		GROUP BY MAHV
		HAVING COUNT(MAHV) <= 3
	  ) AS T 
...
WHERE TRGLOP IN(
 ...
 --CÁCH 2 : SELECT lớp trường từ bảng LOP và kết với bảng KETQUATHI
...
--THÊM:25.1  Tìm họ tên những hoc vien thi không đạt quá 3 môn (mỗi môn đều thi không đạt ở tất cả các lần thi).
...
GROUP BY MAHV
HAVING COUNT(DIEM) = (
                       SELECT MAX(SOLUONG_DIEM)
					   FROM ( 
					          SELECT MAHV , COUNT (DIEM) AS SOLUONG_DIEM
					          FROM KETQUATHI 
							  WHERE DIEM = 9 OR DIEM = 10
							  GROUP BY MAHV 
							) AS T
...
WHERE MAHV IN ( 
               ...
               HAVING COUNT(DIEM) = (
                                      SELECT MAX(SOLUONG_DIEM)
					                  FROM ( SELECT MAHV , COUNT (DIEM) AS SOLUONG_DIEM
					                  FROM KETQUATHI 
							          WHERE DIEM = 9 OR DIEM = 10
							           GROUP BY MAHV ) AS T
				                    )
...
SELECT A.MAHV, L.MALOP, CONCAT(HO,' ',TEN) AS HOTEN
FROM KETQUATHI A, HOCVIEN L
WHERE A.MAHV = L.MAHV AND (DIEM = 9 OR DIEM = 10)
GROUP BY A.MAHV, L.MALOP , HO, TEN
HAVING COUNT(MAMH) >= ALL ( 
                            SELECT COUNT(MAMH)
							FROM KETQUATHI KQ, HOCVIEN HV
							WHERE KQ.MAHV = HV.MAHV AND L.MALOP = HV.MALOP AND (DIEM = 9 OR DIEM = 10) 
							GROUP BY KQ.MAHV 
						  )
...
HAVING COUNT(MALOP) IN (
                          SELECT MAX(SL_LOP)
						  FROM (
						          SELECT COUNT(MALOP) AS SL_LOP
								  FROM GIANGDAY
								  GROUP BY GIANGDAY.MAGV , HOCKY 
							   ) AS T
					    )
...
FROM MONHOC 
INNER JOIN KETQUATHI ON KETQUATHI.MAMH = MONHOC.MAMH
WHERE LANTHI = 1 AND KQUA = 'Khong dat'
GROUP BY  MONHOC.MAMH, TENMH
HAVING COUNT(MONHOC.MAMH) >= ALL (
                                   SELECT COUNT(*)
								   FROM KETQUATHI 
								   WHERE LANTHI = 1 AND KQUA = 'Khong dat'
								   GROUP BY MAMH
...
WHERE NOT EXISTS (
		           ...
		           FROM KETQUATHI
		           WHERE  
		           KQUA = 'Khong dat' AND MAHV = HOCVIEN.MAHV AND LANTHI = 1
	              )
...
--5.Kết quả thi là “Dat” nếu điểm từ 5 đến 10 và “Khong dat” nếu điểm nhỏ hơn 5.
 ...
 ALTER TABLE KETQUATHI 
 ADD CONSTRAINT CHK_KETQUATHI_DIEM_KQTHI CHECK 
                              (	
...
--Cách 2: 
 ALTER TABLE KETQUATHI 
 ADD CONSTRAINT CHK_KETQUATHI CHECK 
                              (	
	                              (KQUA = 'Dat' AND DIEM >= 5)
	                           OR (KQUA = 'Khong dat' AND DIEM < 5)
                              ) 
...
CREATE TRIGGER trg_lop_LOPTRG
ON LOP
FOR INSERT, UPDATE
AS
BEGIN
	DECLARE @MAHV CHAR(5), @LOPTRG CHAR (5), @MALOP CHAR(5)
	...
	SELECT @MAHV = MAHV
	FROM HOCVIEN
	WHERE MAHV = @LOPTRG AND MALOP = @MALOP

	IF (@MAHV = @LOPTRG)
	  PRINT 'Thành công'
	ELSE
	   BEGIN
		  PRINT N'Đã xảy ra lỗi. Lớp trưởng phải là học viên của lớp đó'
...
AS
BEGIN
	DECLARE @MAGV CHAR(5), @TRUONGKHOA CHAR (5), @MAKHOA CHAR(5)
	...
	SELECT @MAGV = MAGV
	FROM GIAOVIEN
	WHERE MAGV = @TRUONGKHOA AND MAKHOA = @MAKHOA AND HOCVI IN ('TS' , 'TSKH')

	IF (@MAGV = @TRUONGKHOA)
	  PRINT 'Thành công'
	ELSE
	   BEGIN
		  PRINT N'Đã xảy ra lỗi. Trưởng khoa phải là giáo viên thuộc khoa'
		  ROLLBACK TRANSACTION
	   END
END
...
AS
BEGIN
	 DECLARE @MAHV CHAR(5), @MAMH_THI CHAR(4), @DENNGAY SMALLDATETIME,  @NGTHI SMALLDATETIME

	SELECT DISTINCT @MAHV = substring(MAHV, 1, 3), @NGTHI = NGTHI, @MAMH_THI = MAMH
	FROM inserted

	SELECT  @DENNGAY = DENNGAY
	FROM GIANGDAY
	WHERE MALOP = @MAHV AND MAMH = @MAMH_THI

	IF (@DENNGAY < @NGTHI)
		PRINT N'THỎA ĐIỀU KIỆN'
	ELSE
		BEGIN
			PRINT N'Học viên chưa học xong môn này'
			ROLLBACK TRANSACTION
	    END
END
...
CREATE TRIGGER SL_MON_HK_NAMHOC
ON GIANGDAY
FOR INSERT, UPDATE
AS
BEGIN
	DECLARE @SL_MON INT, @NAM INT, @HOCKY INT, @MALOP CHAR(3)

	SELECT  @HOCKY = HOCKY, @NAM = NAM, @MALOP = MALOP
	FROM inserted
	...
	ELSE
		BEGIN
			PRINT N'Lớp học này đã học quá 3 môn'
			ROLLBACK TRANSACTION
	    END
END
...
CREATE TRIGGER SL_HOCVIEN_LOP
ON LOP
FOR INSERT, UPDATE
AS
BEGIN
      DECLARE @SL_HV TINYINT, @SISO TINYINT, @MALOP CHAR(3)

	SELECT DISTINCT @SISO = SISO, @MALOP = MALOP
	FROM inserted
	...
	ELSE
	   BEGIN
		  PRINT N'Đã xảy ra lỗi.Sĩ số không bằng số lượng học viên lớp đó'
		  ROLLBACK TRANSACTION
	    END
END
...
BEGIN	
	  ...
	  SELECT DISTINCT @MUCLUONG = A.MUCLUONG, @MAGV = B.MAGV
	   ...
	   WHERE A.HOCHAM = B.HOCHAM AND A.HOCVI = B.HOCVI 
	         AND A.HESO = B.HESO AND A.MAGV <> B.MAGV
	 UPDATE GIAOVIEN
...
BEGIN
	...
	IF ( @LANTHI > 1 )
		BEGIN
			...
			IF ( @DIEM >= 5)
				BEGIN
					PRINT N'Học viên đó đã thi đạt'
				END
		END
...
CREATE TRIGGER Hocvien_THILAI
ON KETQUATHI
FOR INSERT, UPDATE
AS
BEGIN
	DECLARE @MAHV CHAR(5), @LANTHI_1 INT, @NGTHI_1 SMALLDATETIME , @LANTHI_2 INT, @MAMH CHAR(4), @NGTHI_2 SMALLDATETIME 
	
    SELECT @MAHV = MAHV, @LANTHI_1 = LANTHI, @NGTHI_1 = NGTHI, @MAMH = MAMH
	FROM inserted

	SELECT @LANTHI_2 = LANTHI, @NGTHI_2 = NGTHI
	FROM KETQUATHI
	WHERE LANTHI = @LANTHI_1 - 1 
	      AND MAHV = @MAHV 
	      AND MAMH = @MAMH

	IF ( (@LANTHI_1 > 1 and @NGTHI_2 > @NGTHI_1) OR (@LANTHI_1 = 1) )	

	  PRINT N'THỎA ĐIỀU KIỆN'
	ELSE
		BEGIN
			PRINT N'Lỗi.Không thỏa điều kiện'
			ROLLBACK TRANSACTION
	    END
END
...
CREATE TRIGGER phancong_giangday_khoa
ON GIANGDAY
FOR INSERT, UPDATE
AS
BEGIN
	DECLARE @MH_KHOA CHAR(4), @MAMONHOC CHAR(6), @MAGIAOVIEN CHAR(4), @GV_KHOA CHAR(4)

    SELECT @MAGIAOVIEN = MAGV, @MAMONHOC = MAMH
	FROM inserted

	SELECT @MH_KHOA = MAKHOA
	FROM MONHOC
	WHERE MAMH = @MAMONHOC

	SELECT @GV_KHOA = MAKHOA
	FROM GIAOVIEN
